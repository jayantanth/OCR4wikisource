...
if filetype.lower() == "djvu" :
            ...
            if os.path.isfile(filename.split('.')[0] + ".jpg"):
                        logging.info("Found jpg version. Skipping DJVU to jpg conversion")
                        filename = filename.split('.')[0] + ".jpg"
                        filetype = filename.split('.')[-1].lower()
                                                        
            else:
                        
                        message  =  "Found a djvu file. Converting to jpg file. " + "\n\n"
                        logger.info(message)
        
                        command = "ddjvu --format=jpg " +  '"' + filename +  '"' + "   " + '"' +  filename.split('.')[0] + '"' + ".jpg"
...
if filetype.lower() == "jpg":
	...
	# split the jpg files vertically based on the column numbers

        message =  "Aligining the Pages of jpg file. \n"
        logger.info(message)
        command = "mutool poster -x " + str(columns)  + " " + '"' +  filename + '"' +  "  currentfile.jpg"
        logger.info("Running " + command.encode('utf-8'))
        
        os.system(command.encode('utf-8'))
                

        message =  "Spliting the jpg into single jpeg pages. \n"
        logger.info(message)
        convert_command = "Convert currentfile.jpg convert"
        os.system(convert_command)
        logger.info("Running " + convert_command) 
        ...
        counter = 1
        message =  "Joining the jpg files ...\n"
        logger.info(message)

        if columns == "1":
                counter = 1
                for jpg in files:
                        command = "cp " + jpg +  " page_" + str(counter).zfill(5) + ".jpg"
        ...
        if columns == "2":
	        ...
	        for i in chunks:
	            com =  ' '.join(i)
	            command = "jpgunite " + com + " " + "page_" + str(counter).zfill(5) + ".jpg"
	            logger.info("Running " + command) 
	            counter = counter + 1
	            os.system(command)
...
#         pages.append("page_" + str(pageno).zfill(5) + ".jpg")   
...
#Upload the jpg files to google drive and OCR
...
logger.info(message)
command = "mv folder*.log currentfile.jpg  doc_data.txt pg*.jpg page* txt*   " + '"' +  temp_folder + '"'
logger.info("Running " + command.encode('utf-8'))
os.system(command.encode('utf-8'))
...
if  jpg_count == result_text_count:
            logger.info("\n\nThe JPG files and result text files are equval. Now running the mediawiki_uploader.py script\n\n")
